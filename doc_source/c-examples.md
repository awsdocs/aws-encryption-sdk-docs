...
When you create a keyring with multiple CMKs, you specify the CMK used to generate and encrypt the plaintext data key, and an optional array of additional CMKs that encrypt the same plaintext data key\. In this case, you specify only the generator CMK\.   
...
In this example, we specify a keyring configured with the same CMK used to encrypt the string\. Before running this code, replace the example key ARN with a valid one\.  
